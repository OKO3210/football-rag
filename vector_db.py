...
class VectorDB:
    def __init__(self, vector_db_name, chunks=None, metadatas=None):
        ...
        if os.path.exists(vector_db_name):
            ...
            raise Exception(
                "Can't initiate vector db object ! please give a path to a vector db or chunks."
            )
    ...
    def create_vector_db(self, vector_db_name, chunks, metadatas=None):
        ...
        collection = self.chroma.get_or_create_collection(
            name="knowledge", metadata={"embedding_model": EMBEDDING_MODEL_NAME}
        )
...
if __name__ == "__main__":
    ...
    vector_db_object = VectorDB(
        vector_db_name="football_knowledge", chunks=chunks, metadatas=metadatas
    )
